...
static CGFloat const LimitedRotate = M_PI_2/2;// 极限旋转角度
...
@property (nonatomic, assign) CGPoint originalAnchorPoint;// 原始点
@property (nonatomic, assign) CGPoint originalPosition;// 原始位置
@property (nonatomic, assign) CGPoint rotateAnchorPoint;// 旋转点
@property (nonatomic, assign) CGPoint rotatePosition;// 旋转位置
...
#warning 滑动位置
        _beginPoint = [sender locationInView:self.superview];
    }else if(sender.state == UIGestureRecognizerStateChanged){
        CGPoint movePoint = [sender locationInView:self.superview];
        if (_beginPoint.x - movePoint.x > 2) {
            NSLog(@"向左滑动了%f",_beginPoint.x - movePoint.x);
            //让卡片左歪脖
        ...
        if(translation < 0){
            ...
            if(index < self.itemCount){
                if([self.dataSource respondsToSelector:@selector(sweepCardView:itemForIndex:)]){
                    ...
                    self.currentIndex += 1;
                    // 更新显示
                    [_delegate SweepCardView:self updateLabel:(int)self.currentIndex + 1];
